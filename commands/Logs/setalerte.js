...
module.exports = {
    ...
    description: `Permet de changer le salon d'alerte.`,
     async execute(client, message, args,config,emotes,footer) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (alerte.get(`${message.guild.id}.alerteperm`) === newChannel.id) return message.channel.send(`${emotes.owner.alert}・__Nouveau salon d'alerte :__ <#${alerte.get(`${message.guild.id}.alerteperm`)}>`)
            else {
