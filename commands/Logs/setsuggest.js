    ...
    description: `Permet de changer le salon des suggestions.`,
     async execute(client, message, args,config,emote,footer,color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (db.get(`${message.guild.id}.`) === newChannel.id) return message.channel.send(`${emote.logs.question}・__Nouveau salon des suggestions :__ <#${db.get(`${message.guild.id}.suggestions`)}>`)
            else {
