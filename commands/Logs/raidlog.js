    ...
    description: `Permet de changer le salon des logs de raid.`,
     async execute(client, message, args,config,emote,footer,color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (raidlog.get(`${message.guild.id}.raidlog`) === newChannel.id) return message.channel.send(`${emote.buyer.valid}・__Nouveau salon des logs de raid :__ <#${raidlog.get(`${message.guild.id}.raidlog`)}>`)
            else {
