    ...
    description: `Permet de changer le salon des logs de modération.`,
     async execute(client, message, args,config,emote,footer,color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (modlog.get(`${message.guild.id}.modlog`) === newChannel.id) return message.channel.send(`${emote.administration.loading}・__Nouveau salon des logs de modération :__ <#${modlog.get(`${message.guild.id}.modlog`)}>`)
            else {
