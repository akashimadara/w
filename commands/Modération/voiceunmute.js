    ...
    description: `Permet de mute vocal un membre sur le serveur.`,
     async execute(client, message, args,config,emote,footer,color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || message.member.roles.cache.has(perm1) || message.member.roles.cache.has(perm2) || message.member.roles.cache.has(perm3) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if(!muteUser) return message.channel.send({ content: "Veuillez spécifier un membre" })
            if (muteUser.id === message.author.id)
                return message.channel.send("Vous ne pouvez pas vous unmute vous-même.");
            if(muteUser.id === client.user.id) return message.channel.send({ content: "Je ne peux pas me unmute moi-même" })
            if (muteUser.id === message.guild.ownerID) return message.channel.send({ content: "Vous ne pouvez pas unmute le propriétaire du serveur." })

            if (!muteUser.voice.channel) {
                return message.channel.send("Le membre n'est pas dans un salon vocal ou est déjà unmute vocal.")
            }
