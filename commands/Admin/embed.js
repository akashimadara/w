    ...
    async execute(client, message, args, config, emote, footer, color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            message.channel.send({ content: `**Panel de création d'embeds de ${client.user.username}.**` }).then(async d => {
                ...
                const collectorX = msgembed.createMessageComponentCollector({
                    filterSelect,
                    componentType: "BUTTON",
                })
                collectorX.on(`collect`, async (cld) => {
                    if (cld.user.id !== message.author.id) return cld.reply({ content: "Vous n'êtes pas l'auteur de ce message.", ephemeral: true })
                    if(action) return cld.reply({ content: "Une action est déjà en cours", ephemeral: true })
                    action = true;
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                            .then(async (collected) => {
                                ...
                                if (!channel) {
                                 ...
                                 return   cld.message.channel.send({ content: "Salon introuvable." }).then((m) => setTimeout(() => m.delete().catch(() => false), 5000))
                                }
                                ...
                                message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                                    .then(async (collected2) => {
                                        ...
                                        if (!msg) {
                                            ...
                                            return   cld.message.channel.send({ content: "Message introuvable." })
                                        }
                                        if(msg.author.id !== client.user.id) return cld.message.channel.send({ content: "Ce message n'a pas été envoyé par moi." })
                ...
                collector.on(`collect`, async (cld) => {
                    ...
                    if (value === "embedtitle") {
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                            .then(async (collected) => {
                                action = false;
                                if (collected.first().content.length > 64) return cld.message.channel.send({ content: "Nom trop long." }).then(async z => setTimeout(()=> {
                                    z.delete().catch(() => false)
                                    yx.delete().catch(() => false)
                                }, 2000))
                                action = true;
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                            .then(async (collected) => {
                                action = false;
                                if (collected.first().content.length > 64) return cld.message.channel.send({ content: "Texte trop long." }).then(async z => setTimeout(()=> {
                                    z.delete().catch(() => false)
                                    yx.delete().catch(() => false)
                                }, 2000))
                                action = true;
                        ...
                        message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                            .then(async (collected) => {
                                action = false;
                                if (collected.first().content.length > 128) return cld.message.channel.send({ content: "Nom trop long." }).then(msg=> {
                                    setTimeout(() => {
                                ...
                                message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                                    .then(async (collected2) => {
                                        action = false;
                                        if (embed.fields.length === 25) return cld.message.channel.send({ content: "Il y a trop de fields sur cet embed." }).then(msg=> {
                                            setTimeout(() => {
                                                msg.delete().catch(() => false)
                                                yxy.delete().catch(() => false);
                                                yx.delete().catch(() => false);
