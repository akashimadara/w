     ...
     async execute(client, message, args,config,emote,footer,color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
                ...
                if((message.member.id !== message.guild.ownerId) && (message.member.roles.highest.position < role.position)) return message.channel.send(`Vous ne pouvez pas retiré ce rôle à ce membre`)
                if(message.guild.members.me.roles.highest.position < role.position) return message.channel.send(`Je ne peux pas retiré ce rôle à ce membre`)
                
                if (!role) return message.channel.send(`Aucun rôle trouvé pour \`${args[1] || "rien"}\``)

                await member.roles.remove(role.id, `Rôle retiré par ${message.author.tag}`).then(()=> message.channel.send({content : "1 rôle retiré à 1 membre"})).catch(() => message.channel.send(`Je ne peux pas retiré ce rôle à ce membre`))
