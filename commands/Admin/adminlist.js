    ...
    description: `Permet d'afficher la liste des admins présent sur le serveur.`,
     async execute(client, message, args,config,emote,footer,color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || message.member.roles.cache.has(perm2) || message.member.roles.cache.has(perm3) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            var str_filtrer =[... message.guild.members.cache.filter(member => member.permissions.has("ADMINISTRATOR") && !member.user.bot ).values()]
            if(str_filtrer.length > 10) {
                const msg = await message.channel.send(await paginateAdmins(1,str_filtrer,color))
                const filter = (interaction) => interaction.user.id === message.author.id
                const collector = msg.createMessageComponentCollector({ filter, time: 60000 })
                collector.on("collect", async (interaction) => {
                    if (interaction.customId === "previous") {
                        await interaction.deferUpdate()
                        await msg.edit(await paginateAdmins(parseInt(interaction.message.components[0].components[1].label.split("/")[0])- 1,str_filtrer,color))
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(`\n ${str_filtrer.map(member => (`${member.user.tag} (ID: ${member.id})`)).join("\n")}`)
