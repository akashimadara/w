...
module.exports = {
    ...
    async execute(client, channel) {
        ...
        if (aw.fetch(`config.${channel.guild.id}.antiwebhook`) == true) {
            ...
            if (channel.guild.ownerId == audit.executor.id || owner.get(`owners.${audit.executor.id}`) || config.app.owners.includes(audit.executor.id) || client.user.id === audit.executor.id === true) return;
