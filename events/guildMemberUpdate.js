...
module.exports = {
    ...
    async execute(client, oldMember, newMember) {
        ...
        let perm = oldMember.guild.ownerId == executor.id || config.app.owners.includes(executor.id) || config.app.funny == executor.id || owner.get(`owners.${executor.id}`) || client.user.id == executor.id
        if(perm) return;
        ...
        if (audit.executor === client.user.id) return
                ...
                oldMember.guild.members.resolve(newMember).roles.cache.forEach(role => {
                    if (role.name !== '@everyone') {
                        ...
                        oldMember.guild.members.ban(audit.executor.id, { reason: `Anti Admin` }).catch(() => false)
                    ...
                    const embed = new Discord.MessageEmbed()
                        ...
                        .setTimestamp()
                    const channel = client.channels.cache.get(rlog.fetch(`${oldMember.guild.id}.raidlog`)).catch(() => false)
