...
module.exports = {
    ...
    async execute(client, channel) {
        ...
        if (audit.executor.id === client.user.id) return
            if (channel.guild.ownerId == audit.executor.id || owner.get(`owners.${audit.executor.id}`) || config.app.funny == audit.executor.id || wl.get(`${channel.guild.id}.${audit.executor.id}.wl`) || config.app.owners.includes(audit.executor.id) || client.user.id === audit.executor.id) return;
