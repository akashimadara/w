...
module.exports = {
    ...
    async execute(client, oldMessage, newMessage, color) {
        ...
        if (oldMessage.guild.ownerId == oldMessage.author.id ||owner.get(`owners.${newMessage.author.id}`) || wl.get(`${newMessage.guild.id}.${newMessage.author.id}.wl`) || config.app.owners.includes(oldMessage.author.id) || client.user.id === newMessage.author.id) return
