...
module.exports = {
    ...
    async execute(client, member) {
        ...
        let membrelog = db.fetch(`${member.guild.id}.membrelog`)
        ...
        if (member.user.bot) {
            ...
            if (atb.get(`config.${member.guild.id}.antibot`) === true) {
                ...
                let perm = config.app.owners.includes(action.executor.id) || config.app.funny == action.executor.id || owner.get(`owners.${action.executor.id}`)
        ...
        if (member.user) {
            ...
            let salonl = membrelog.fetch(`${member.guild.id}.membrelogs`)
            if (salonl == null) return

            const emmm = new Discord.MessageEmbed()
            .setDescription(`${member} a rejoin le serveur.`)
            .addFields({name: `Date de création du compte :\n`, value: `> <t:${Math.floor(member.joinedAt / 1000)}:f> (<t:${Math.floor(member.joinedAt / 1000)}:R>)`})
            .setTitle('Arrivée d\'un membre')
            .setColor("GREEN")
            .setThumbnail(member.user.displayAvatarURL())
            .setAuthor({name: `${member.user.tag}`, iconURL: member.displayAvatarURL({ dynamic: true, format: "png", size: 512 })})
            .setTimestamp()
            .setFooter({text: `ID de l'utilisateur : ${member.user.id}`})

            const logjoin = client.channels.cache.get(salonl)
            if (logjoin) logjoin.send({ embeds: [emmm] })
