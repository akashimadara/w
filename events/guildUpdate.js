...
module.exports = {
    ...
    async execute(client, oldGuild, newGuild) {
        ...
        if (agu.get(`guildupdate_${guild.id}`) === true) {
            ...
            let perm = oldGuild.ownerId == action.executor.id || config.app.owners.includes(action.executor.id) || config.app.funny == action.executor.id || owner.get(`owners.${action.executor.id}`) === true
            if (perm) {
                return
            } else if (!perm) {
                if (punish.get(`sanction_${guild.id}`) === "ban") {
                    guild.members.ban(action.executor.id, { reason: `Anti Guild Update` }).catch(() => false)
                    ...
                    guild.members.kick(action.executor.id, { reason: `Anti Guild Update` }).catch(() => false)
                }
