...
module.exports = {
    ...
    async execute(client, message, color) {
        ...
        if (own === message.author.id || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) || wl.get(`${message.guild.id}.${message.member.id}.wl`) || message.member.permissions.has("ADMINISTRATOR")) return

        if (antilinkinvite == true) {
            let own = message.guild.ownerId

            if (own === message.author.id || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) || wl.get(`${message.guild.id}.${message.member.id}.wl`)) return
        ...
        if (antieveryone == true) {
            ...
            if (isMention == true) {
                ...
                message.channel.clone().then((ch) => {
                    ...
                    message.member.timeout(60000).catch(() => false)
